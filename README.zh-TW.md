...
- 介面支援 **繁體中文、简体中文、English、日本語** 四種語言,可隨時切換。
...
- **點擊穿透模式**:🖱 一鍵讓滑鼠「穿過」字幕視窗直接操作底下的影片;
  按住 **Ctrl+Alt** 可暫時操作視窗(拖曳、按按鈕)。
- **暫停/繼續**:⏸ 暫停期間完全不呼叫 API,不計費。
- **無音訊偵測**:工具列音訊指示燈(綠=有聲/灰=靜音/紅=長時間無聲),
  30 秒沒收到聲音自動提示檢查 CABLE 路由。
- **術語對照表**:人名/術語固定譯法(如 VTuber 名字),清單空白時對翻譯零影響。
- **記住視窗位置**:關閉自動儲存位置與大小,下次開啟回到原位。
...
3. 跟著**首次設定精靈**走:選介面語言 → 填 OpenAI API Key(按「測試連線」驗證)→ 選 CABLE 裝置 → 完成。
...
| `i18n.py`            | 介面語言字典(繁中/簡中/English/日本語) |
| `theme.py`           | UI 配色與字型常數 |
...
| 參數 | 預設 | 說明 |
|------|------|------|
| 介面語言 | 繁體中文 | 繁中/簡中/English/日本語;**與翻譯目標語言無關**,切換後即時生效 |
...
| 來源/目標語言 | 日文 / 繁體中文 | |
| 術語對照表 | (空) | 一行一組「原文=譯文」,`#` 開頭為註解;空白時零影響 |
...
| 沒有任何字幕 | 看工具列音訊指示燈:紅色=聲音沒進來,確認聲音有導到 `CABLE Input`;詳見 `%APPDATA%\YoutubeLiveTranslator\translator.log` |
| 開了點擊穿透後點不到視窗 | 按住 **Ctrl+Alt** 即可暫時操作(再按 🖱穿透 按鈕關閉) |
